...
function getDisplayEndTime(event: ScheduleEvent): Date {
  return event.endTime ?? new Date(event.startTime.getTime() + POINT_IN_TIME_DURATION_MINUTES * 60_000);
}

function eventsOverlap(a: ScheduleEvent, b: ScheduleEvent): boolean {
  return a.startTime < getDisplayEndTime(b) && b.startTime < getDisplayEndTime(a);
}
...
function positionDayEvents(
  ...
  for (const event of sorted) {
    const prevOverlapping = result.filter((p) => eventsOverlap(p, event));
    const usedCols = prevOverlapping.map((p) => p.column);
    const column: EventColumn = usedCols.includes(0) ? 1 : 0;
    const eventHasOverlap = sorted.some((o) => o.id !== event.id && eventsOverlap(o, event));
    const isPointInTime = event.endTime === null;
    const displayEndTime = getDisplayEndTime(event);
    const visibleStart = event.startTime < dayStart ? dayStart : event.startTime;
    const visibleEnd = displayEndTime > dayEnd ? dayEnd : displayEndTime;
    const startMins = getMinutesFromDayStart(visibleStart);
    const endMins = getMinutesFromDayStart(visibleEnd);
...
function getDayLabel(dayKey: string): string {
  ...
  return SHORT_WEEKDAYS[(date.getUTCDay() + 6) % 7];
}

function getDayNumber(dayKey: string): number | string {
  const date = parseFloatingDate(dayKey);
...
function EventDialog({
  ...
  const displayEnd = getDisplayEndTime(event);
  const isPoint = event.endTime === null;
  ...
  const timelineStartHour = useMemo(() => {
    ...
    for (const dayKey of weekDays) {
      ...
      if (dayEvents.length > 0) {
        const h = getTimelineStartHour(dayEvents, dayKey);
        if (h < min) min = h;
      }
    }
    return min;
        ...
        <div className="overflow-x-auto">
            ...
            {weekDays.map((dayKey) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-sm font-semibold">{getDayNumber(dayKey)}</div>
                </div>
              );
            })}
          </div>
          ...
          {hasAnyEvents && (
            ...
            >
              ...
              {/* Day columns */}
              {weekDays.map((dayKey) => {
                const dayEvents = positionedByDay[dayKey] ?? [];
                return (
                  <div
                    key={dayKey}
