...
function getEventDisplayEndTime(
	event: ScheduleEvent,
) {
	return (
		event.endTime ??
		new Date(
			event.startTime.getTime() +
				POINT_IN_TIME_EVENT_DURATION_MINUTES *
					60_000,
		)
	)
}
...
function hasScheduleOverlap(
	...
	return (
		a.startTime <
			getEventDisplayEndTime(b) &&
		b.startTime <
			getEventDisplayEndTime(a)
	)
}
...
function positionEvents(
	...
	for (const event of sortedEvents) {
		const previousOverlappingEvents =
		...
		const usedColumns =
			previousOverlappingEvents.map(
				(previousEvent) =>
					previousEvent.column,
			)
		const column: EventColumn =
			usedColumns.includes(0)
				? 1
				: 0

		const hasTooManyOverlaps =
			previousOverlappingEvents.length >=
			MAX_OVERLAPPING_EVENTS
		const hasOverlap =
			sortedEvents.some(
				(otherEvent) =>
					otherEvent.id !==
...
export function ScheduleTimeline({
	...
	const hours = Array.from(
		{
			length:
				24 - timelineStartHour,
		},
		(_, index) =>
			timelineStartHour + index,
	)
